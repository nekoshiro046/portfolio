...
let font;
var ww = 5,wh = 5;
function preload(){font = loadFont('assets/font/FreeSans.otf');}
function setup() {
  ...
  textFont(font);textSize(fontSize);textAlign(CENTER, CENTER);textStyle(NORMAL);	rectMode(CENTER);
}
function draw(){drawScene();}
function windowResized(){resizeCanvas(windowWidth/ww, windowWidth/ww);}
function mousePressed() {if(firstBox.inTerritory(mouseX,mouseY) && inCanvas()){window.close();}}
function drawScene(){
  background(255);
...
function drawText(x,y,str,strSiz,col){push();  translate(x,y);fill(col);textSize(strSiz);text(str, 0,0);pop();}
function inCanvas(){
	var back;
	if(mouseX < 0 || mouseX > windowWidth || mouseY < 0 || mouseY > windowHeight){back = false;}else{back = true;}
	return back;
}
function objInCanvas(x,y){
	var back;
	if(x <  -windowWidth/2|| x > windowWidth/2|| y < -windowHeight/2 || y > windowHeight/2){back = false;}else{back = true;}
	return back;
}
class box{
	...
	moveInnerBox(){
		...
		if(rnX < 1){pointx1 = this.core.x - this.inch/2 + this.inch/2;pointx2 = this.core.x + this.inch/2 + this.inch/2;
		}else{pointx1 = this.core.x - this.inch/2 - this.inch/2;pointx2 = this.core.x + this.inch/2 - this.inch/2;}
		if(rnY < 1){pointy1 = this.core.y - this.inch/2 + this.inch/2;pointy2 = this.core.y + this.inch/2 + this.inch/2;
		}else{pointy1 = this.core.y - this.inch/2 - this.inch/2;pointy2 = this.core.y + this.inch/2 - this.inch/2;}
		if(rnZ < 1){pointz1 = this.core.z - this.inch/2 + this.inch/2;pointz2 = this.core.z + this.inch/2 + this.inch/2;
		}else{pointz1 = this.core.z - this.inch/2 - this.inch/2;pointz2 = this.core.z + this.inch/2 - this.inch/2;}
		for(let i = 0; i <= 7; i++){
	      switch(i){
	        case 0:
	...
	inTerritory(mx,my){
	    ...
	    if(dist_box < this.territory){back = true;}else{back = false;}
	    return back;
	}
}
